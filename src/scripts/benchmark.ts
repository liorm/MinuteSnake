...
function printBenchmarkResults(results: BenchmarkResult[], verbose: boolean): void {
  console.log('\n⚡ Benchmark Results:');
  console.log('=' .repeat(100));
  
  // Header
  console.log('Network Name'.padEnd(30) + 
  ...
  const sortedResults = results.sort((a, b) => a.averageInferenceTime - b.averageInferenceTime);
  ...
  if (verbose) {
    ...
    for (const result of sortedResults) {
      ...
      const stdDev = Math.sqrt(times.reduce((sum, time) => sum + Math.pow(time - result.averageInferenceTime, 2), 0) / times.length);
  ...
  if (results.length > 0) {
    ...
    console.log(`Fastest Network: ${results[0].name} (${results[0].averageInferenceTime.toFixed(3)}ms)`);
    console.log(`Slowest Network: ${results[results.length - 1].name} (${results[results.length - 1].averageInferenceTime.toFixed(3)}ms)`);
    console.log(`Average Inference Time: ${(avgTimes.reduce((a, b) => a + b, 0) / avgTimes.length).toFixed(3)}ms`);
    console.log(`Average Throughput: ${(avgThroughput.reduce((a, b) => a + b, 0) / avgThroughput.length).toFixed(0)} ops/s`);
    console.log(`Average Memory Usage: ${(avgMemory.reduce((a, b) => a + b, 0) / avgMemory.length).toFixed(1)}MB`);
  }
  ...
  console.log(`Inference Time < ${targetInferenceTime}ms: ${meetsTiming ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`Throughput > ${targetThroughput} ops/s: ${meetsThroughput ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`Memory Usage < ${targetMemory}MB: ${meetsMemory ? '✅ PASS' : '❌ FAIL'}`);
...
async function saveBenchmarkResults(results: BenchmarkResult[], filename: string): Promise<void> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (args.weights) {
      ...
      const name = args.weights.split('/').pop()?.replace('.json', '') || 'unknown';
      
      const result = await benchmarkNetwork(name, weights, iterations, warmupIterations, args.verbose || false);
      results.push(result);
    }
    // Benchmark all weight files in directory
      ...
      for (const weightFile of weightFiles) {
        ...
        const name = weightFile.split('/').pop()?.replace('.json', '') || 'unknown';
