...
export class WeightLoader {
  ...
  public loadFromObject(weightData: WeightFileFormat): WeightLoadResult {
    ...
    if (!this.validateArchitectureConsistency(weightData.metadata, result.architecture, result)) {
      return result;
    }
  ...
  public async loadFromFile(_filePath: string): Promise<WeightLoadResult> {
    try {
      ...
      throw new Error('File loading not implemented - use loadFromJSON with file contents');
    } catch (error) {
      return {
        architecture: { layers: [] },
        metadata: { architecture: [] },
   ...
   */
  public saveToJSON(architecture: NetworkArchitecture, metadata: WeightMetadata): string {
  ...
  private validateVersion(version: string, result: WeightLoadResult): boolean {
    ...
    if (!supportedVersions.includes(version)) {
      result.errors.push(`Unsupported version: ${version}. Supported versions: ${supportedVersions.join(', ')}`);
      return false;
    }
   ...
   */
  private validateMetadata(metadata: WeightMetadata, result: WeightLoadResult): boolean {
    if (!metadata) {
    ...
    if (metadata.architecture.length < 2) {
      result.errors.push('Architecture must have at least 2 layers (input and output)');
      return false;
    }

    if (metadata.architecture.some(size => !Number.isInteger(size) || size <= 0)) {
   ...
   */
  private validateAndConvertLayers(layerData: WeightLayerData[], result: WeightLoadResult): boolean {
   ...
   */
  private validateLayerStructure(layer: WeightLayerData, index: number, result: WeightLoadResult): boolean {
   ...
   */
  private validateLayerDimensions(layer: WeightLayerData, index: number, result: WeightLoadResult): boolean {
    // Validate size fields
    if (!Number.isInteger(layer.inputSize) || layer.inputSize <= 0) {
      result.errors.push(`Layer ${index}: inputSize must be a positive integer`);
      return false;
    }

    if (!Number.isInteger(layer.outputSize) || layer.outputSize <= 0) {
      result.errors.push(`Layer ${index}: outputSize must be a positive integer`);
      return false;
    }
    ...
    if (layer.weights.length !== layer.outputSize) {
      result.errors.push(`Layer ${index}: weights array length (${layer.weights.length}) must match outputSize (${layer.outputSize})`);
      return false;
    }
    ...
    for (let neuronIdx = 0; neuronIdx < layer.weights.length; neuronIdx++) {
      ...
      if (!Array.isArray(neuronWeights)) {
        result.errors.push(`Layer ${index}, neuron ${neuronIdx}: weights must be an array`);
        return false;
      }

      if (neuronWeights.length !== layer.inputSize) {
        result.errors.push(`Layer ${index}, neuron ${neuronIdx}: weights length (${neuronWeights.length}) must match inputSize (${layer.inputSize})`);
        return false;
      }
    }
    ...
    if (layer.biases.length !== layer.outputSize) {
      result.errors.push(`Layer ${index}: biases array length (${layer.biases.length}) must match outputSize (${layer.outputSize})`);
      return false;
    }
   ...
   */
  private validateWeightValues(layer: WeightLayerData, index: number, result: WeightLoadResult): boolean {
    // Check weights
    for (let neuronIdx = 0; neuronIdx < layer.weights.length; neuronIdx++) {
      const neuronWeights = layer.weights[neuronIdx];
      ...
      for (let weightIdx = 0; weightIdx < neuronWeights.length; weightIdx++) {
        ...
        if (typeof weight !== 'number' || !isFinite(weight)) {
          result.errors.push(`Layer ${index}, neuron ${neuronIdx}, weight ${weightIdx}: must be a finite number`);
          return false;
        }

        if (Math.abs(weight) > this.config.maxWeightMagnitude) {
          if (this.config.strictValidation) {
            result.errors.push(`Layer ${index}, neuron ${neuronIdx}, weight ${weightIdx}: magnitude (${Math.abs(weight)}) exceeds limit (${this.config.maxWeightMagnitude})`);
            return false;
          } else {
            result.warnings.push(`Layer ${index}, neuron ${neuronIdx}, weight ${weightIdx}: large magnitude (${Math.abs(weight)})`);
          }
        }
      }
    }
    ...
    for (let biasIdx = 0; biasIdx < layer.biases.length; biasIdx++) {
      ...
      if (typeof bias !== 'number' || !isFinite(bias)) {
        result.errors.push(`Layer ${index}, bias ${biasIdx}: must be a finite number`);
        return false;
      }

      if (Math.abs(bias) > this.config.maxWeightMagnitude) {
        if (this.config.strictValidation) {
          result.errors.push(`Layer ${index}, bias ${biasIdx}: magnitude (${Math.abs(bias)}) exceeds limit (${this.config.maxWeightMagnitude})`);
          return false;
        } else {
          result.warnings.push(`Layer ${index}, bias ${biasIdx}: large magnitude (${Math.abs(bias)})`);
        }
      }
    }
   ...
   */
  private validateLayerConnectivity(layers: NetworkLayer[], result: WeightLoadResult): boolean {
    for (let i = 1; i < layers.length; i++) {
      const prevLayer = layers[i - 1];
      const currentLayer = layers[i];
      
      if (prevLayer.outputSize !== currentLayer.inputSize) {
        result.errors.push(`Layer connectivity error: layer ${i - 1} output (${prevLayer.outputSize}) does not match layer ${i} input (${currentLayer.inputSize})`);
        return false;
      }
    }
  ...
  private validateArchitectureConsistency(
    ...
    if (actualArchitecture.length !== expectedArchitecture.length) {
      if (this.config.allowArchitectureMismatch) {
        result.warnings.push(`Architecture mismatch: expected ${expectedArchitecture.length} layers, got ${actualArchitecture.length}`);
      } else {
        result.errors.push(`Architecture mismatch: expected ${expectedArchitecture.length} layers, got ${actualArchitecture.length}`);
        return false;
      }
    }

    for (let i = 0; i < Math.min(actualArchitecture.length, expectedArchitecture.length); i++) {
      if (actualArchitecture[i] !== expectedArchitecture[i]) {
        if (this.config.allowArchitectureMismatch) {
          result.warnings.push(`Architecture mismatch at layer ${i}: expected ${expectedArchitecture[i]}, got ${actualArchitecture[i]}`);
        } else {
          result.errors.push(`Architecture mismatch at layer ${i}: expected ${expectedArchitecture[i]}, got ${actualArchitecture[i]}`);
          return false;
        }
      }
    }
    ...
    for (let layerIdx = 0; layerIdx < layers.length; layerIdx++) {
      ...
      for (let neuronIdx = 0; neuronIdx < layer.outputSize; neuronIdx++) {
        ...
        if (totalMagnitude < 1e-6) {
          result.warnings.push(`Layer ${layerIdx}, neuron ${neuronIdx}: appears to be dead (very small weights and bias)`);
        }
      }
    }
   ...
   */
  private normalizeArchitectureWeights(architecture: NetworkArchitecture): void {
